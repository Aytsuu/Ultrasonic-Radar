...
void draw() {
  ...
  fill(98, 245, 31);
  // simula  ting motion blur and slow fade of the moving line
  noStroke();
  fill(0, 4); 
...
void serialEvent(Serial myPort) {
  try {
    ...
    index1 = data.indexOf(",");
    if (index1 < 0) return; // Skip if no comma
...
void drawRadar() {
  ...
  // Draw angle lines for 30° increments (adjust as needed)
  for (int i = 15; i <= 165; i += 30) {
